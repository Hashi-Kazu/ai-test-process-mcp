...
// input-quality-criteria（IQC-01..IQC-05）の判定区分カタログ。
...
export const inputQualityCriteria: InputQualityCriteria = {
  ...
  criteria: [
    ...
    },
    {
      ...
      nameJa: "双方向制御文字の混入",
      severity: "medium",
      metric: "本文に含まれる双方向制御文字（U+202A〜U+202E, U+2066〜U+2069, U+200E, U+200F）の出現数と符号位置別内訳。",
      threshold: "1文字でも検出したら medium。比率ではなく実数で判定する（可視文字を持たず、少数でも行頭判定・照合を破壊するため）。",
      description:
        "PDF/Word からのテキスト抽出で、表示方向制御のための不可視文字が本文へそのまま混入することがある。" +
        "可視文字を持たないため目視では気付けず、行頭マーカー判定（ID定義／参照の役割判定）と逐語引用の照合を破壊する。",
      action:
        "本ツールは検出した双方向制御文字を除去してから全検査を実施する。件数は入力ダイジェストへ出力する。原本の変換設定を見直すことが望ましい。",
      measuredEvidence: [
        "2024年版テストベース 11号（pdftotext -layout, poppler 24.02.0）で 3,858 字。同年度の他8文書および2025年版9文書は0字。",
        "除去前は 11号 のID定義49件がすべて参照と判定され、定義ID総数が 162→113、未解決参照が 5→132 になっていた。",
      ],
    },
  ],
  notes: [
    ...
    "IQC-05は達成度・網羅率を示す指標ではなく、除去した実文字数そのものを提示する実測値である。宣言側の値を持たないため" +
      "宣言と実体の照合は成立しないが、代わりに符号位置別内訳という実体の内訳を必ず併記する。",
  ],
};
