...
export const testDataDesignCriteria: TestDataDesignCriteria = {
  ...
  categories: [
    ...
    },
    {
      ...
      severity: "medium",
      definition:
        "要求が指すデータ区分名・状態名（dataItemId 指定時はラベル・属性値も含む）のいずれもが、そのケースの前提条件・手順本文に現れない。ケース本文が1つも渡されていないケースは判定しない。",
      recommendedAction: "データ区分名・状態名（または実体のラベル・属性値）が本文から読み取れるよう前提条件・手順を補記すること。",
    },
    {
      id: "TDC-12",
      nameJa: "供給過剰",
  ...
  notes: [
    ...
    "被覆率は本文裏付け(TDC-11)を通過した要求だけを分子に数える。テストケース本文が1件も渡されていない、または遷移が1件も宣言されていない場合は、数値を出さず未算出（理由）と明記する。",
