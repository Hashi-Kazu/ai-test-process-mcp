...
ユーザーが日常的に作成するテスト仕様書（テスト設計仕様・テストケース仕様）のレビュー tool を追加する。

- **新規ファイル**:
  - `src/resources/testSpecReviewChecklist.ts`: JSTQB のテスト分析・設計観点に基づくチェックリスト。
    - テスト条件の網羅性、期待結果の明確さ、テストベースへのトレーサビリティ、テスト技法の適切さ、事前条件・手順の実行可能性 など。
  - `src/tools/reviewTestSpec.ts`: tool `review_test_spec`。入力はテスト仕様書の Markdown 本文（任意でテストベース要約を併せて受け取る）。Phase 3 と同じ二層構成。
    - 構造検査の例: テストケースIDの重複、期待結果の空欄 など。
- **完了条件**:
  - 構造検査の vitest 単体テストが PASS。
  - MCP Inspector 経由で `review_test_spec` の呼び出しが動作すること。
...
| Phase 2 | JSTQB用語基盤（glossary resource + テンプレート用語整合） | 小 | 未着手 |
| Phase 3 | テスト計画書レビュー tool | 中 | 未着手 |
| Phase 4 | テスト設計・テストケース仕様書レビュー tool | 中 | 未着手 |
