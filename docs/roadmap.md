...
- **新規ファイル**:
  ...
  - `src/tools/reviewTestSpec.ts`: tool `review_test_spec`。**入力はテストベースの Markdown 本文とテスト仕様書の Markdown 本文の2つ（いずれも必須）**。Phase 3 と同じ二層構成。
    - 構造検査（決定的）の例: テストケースIDの重複、期待結果の空欄、要件ID（テストベース側にID表記がある場合）とテストケースの対応表を機械的に構築し、どのテストケースからも参照されていない要件IDを未カバー候補として列挙 など。
