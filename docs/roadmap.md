...
| M5-2 出力量 | 1ツールの既定出力を呼び出し元 context に載る規模へ。要約表示が重複ID件数に比例して破綻する構造を解消する | 実装済み（`analyze_requirements`。2.1/2.6/3章に件数上限を導入し、打ち切り箇所には全件数・省略件数・`verbose: true` の案内を明示。GitHub Issue #205） |
| M5-3 優先順位付け | 決定的指摘へ対処優先度を機械的に付与する。曖昧語等の定型表現による偽陽性を、除外件数と規則IDを明示したうえで抑制する | 実装済み（`analyze_requirements` / `review_test_basis` / `audit_basis_contradictions` / `audit_id_population`。severity は判定区分ごとの固定値のまま、影響ID数・文書横断性・章節解決可否から対処優先度スコア（最大55点・P1〜P4）を算出して併記し、指摘一覧をスコア降順・既定上位20件・残り件数明示で返す。宣言severityと実体因子の逆転も併記する。GitHub Issue #208） |
| M5-4 入力コスト | 上流出力→下流payloadの引き渡しJSONを標準化し、interview prompt と原文入力口を拡張する | 着手済み（引き渡しJSONの標準化。`extract_test_conditions` → `design_test_architecture` / `generate_test_cases` / `audit_cross_matrix`、`design_test_architecture` → `analyze_execution_order` / `audit_cross_matrix`、`design_scenario_flows` → `generate_test_cases` の5経路6ペイロードを `## 10. 下流ツール引き渡しJSON` として実装。上流の算出済み実体からペイロードを組み立て、受け側の算出ロジック（`computeExecutionOrder` / `analyzeCrossMatrix` / `buildScenarioFlowCoverageTargets` / `findUnresolvedDerivedFromRefs`）へ通し直して往復照合し、判定区分 `HPO-01`〜`HPO-05` で不一致を出す。JSON本文は `emitHandoverPayload: true` のオプトイン。共通基盤 `src/handoverPayload.ts`。残: design_* 系→`audit_cross_matrix` の残り経路。GitHub Issue #210 / Jira `HSKZ-214`）<br>着手済み（interview prompt の拡張。構造化JSONを手組みで要求する主要ツール向けに7本を追加し、interview prompt を5本→12本へ拡張。追加分は `test_condition_interview`（`extract_test_conditions`）/ `test_architecture_interview`（`design_test_architecture`）/ `scenario_flow_interview`（`design_scenario_flows`）/ `test_specification_review_interview`（`review_test_specification`）/ `id_population_audit_interview`（`audit_id_population`）/ `threshold_change_interview`（`reexpand_threshold_changes`）/ `data_flow_timing_interview`（`analyze_data_flow_timing`）。共有純関数 `src/prompts/interviewTopic.ts`（`extractCollectToRootKeys` / `buildInterviewPromptText`）を新設し、既存5本は変更していない。あわせて全12本について「promptの収集先が対象ツールzodスキーマのキーに実在する（prompt→schema）」と「zodスキーマの必須キーが必須topicで全件カバーされている（schema→prompt）」の双方向一致を `test/interviewPromptSchemaAlignment.test.ts` で機械検証する。残: design_* 系→`audit_cross_matrix` の残り経路。GitHub Issue #211 / Jira `HSKZ-215`）<br>着手済み（原文入力口の拡張。`design_scenario_flows` / `design_test_data` / `design_config_matrix` / `design_decision_table` / `design_pairwise` / `design_test_architecture` / `analyze_data_flow_timing` / `select_regression_suite` の8ツールへ optional `testBasisDocuments` を追加し、原文入力口を持つツールを 10/31 → 18/31 へ拡張（`tools/list` の `inputSchema` から機械的に数えて検証）。共有純関数 `src/testBasisGrounding.ts` を新設し、宣言したラベル（因子名・水準値・状態名・アクター名・コンテナ名等）・鍵括弧引用・明示的な外部ID・出典文書名がテストベース本文（`sourceRef.document` は投入文書名）に実在するかを、正規化後の包含判定で決定的に照合して判定区分 `TBG-01`〜`TBG-04` として出す（`## テストベースとの実在照合` 節。各ツール既存の `findings` へはマージせず、`severity: high` による生成打ち切りの挙動を変えない）。自由記述文の逐語全文照合は行わず、括弧引用のみを対象とすることと、`testBasisDocuments` 未指定時は無言合格ではなく「検査不能（要確認）」であることを本文と `## 検査実行状況` 対照表の両方に明示する（`inspectabilityCatalog` を11ツール→19ツールへ拡張）。残: design_* 系→`audit_cross_matrix` の残り経路。GitHub Issue #212 / Jira `HSKZ-216`） |
